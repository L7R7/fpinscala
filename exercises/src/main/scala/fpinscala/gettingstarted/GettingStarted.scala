...
object MyModule {
  ...
  private def formatAbs2(x: Int) = s"The absolute value of $x is ${abs(x)}"

  def main(args: Array[String]): Unit = {
    println(formatAbs(-42))
    println(formatAbs2(-42))
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    def go(i: Int): Boolean =
      if (i == as.length - 2) gt(as(i), as(as.length - 1))
  ...
  def isSorted2[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    def go(i: Int): Boolean =
      if (i == as.length - 1) true // if it got so far, the whole array is sorted
      else if (gt(as(i), as(i + 1))) go(i + 1)
      else false
  ...
  def main(args: Array[String]): Unit = {
    val as = Array(4, 3, 2, 1)
    println(isSorted(as, (a: Int, b: Int) => a > b))
    println(isSorted2(as, (a: Int, b: Int) => a > b))
  }
