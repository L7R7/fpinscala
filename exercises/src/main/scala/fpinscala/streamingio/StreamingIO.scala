...
input (`Await`) or signaling termination via `Halt`.
  ...
  object Process {
    ...
    def sum2: Process[Double, Double] = loop(0.0)((d: Double, d1: Double) => (d + d1, d + d1))

    def count3[I]: Process[I, Int] = loop(0)((_: I, i: Int) => (i + 1, i + 1))
    ...
    def convertFahrenheitFile(f: java.io.File): IO[String] =
      processFile(f, convertFahrenheit, "No temperatures found")((s1: String, s2: String) => if (s2.isEmpty) s1 else s2)
  }
}
